...
class Game {
  ...
  checkEndOfBattle() {
    if (this.player.isAlive() && this.currentEnemy.isAlive()) {
      ...
      // Create an inquirer prompt that asks players if they want to continue or quickly go to the shop
  ...
  // merchantShop() activates when users go to shop.
  // merchantShop() {
    // Announce player GP
    // console.log(`${this.player.name} has won ${this.currentEnemy.gp} for beating the ${this.currentEnemy.name}`);
    // Have a category of items players can buy
      // weapons (boosts strength)
      // armor (boosts AC)

      // If the player already owns a similar item, prevent from purchasing.
      // Boost player stats based on purchased item.
    // Leave and increment round when done
  // }
