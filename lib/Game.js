...
class Game {
  constructor() {
    ...
    this.weaponData = [
      { name: 'Weapon1', price: 10 + 'GP', damage: 15, Attack_Bonus: '+' + 2, category: 'Weapon'},
      { name: 'Weapon2', price: 20 + 'GP', damage: 25, Attack_Bonus: '+' + 4, category: 'Weapon'},
    ];
    this.armorData = [
      { name: 'ArmorSet1', price: 30 + 'GP', Armor_Bonus: '+' + 2, category: 'Armor'},
      { name: 'ArmorSet2', price: 50 + 'GP', Armor_Bonus: '+' + 4, category: 'Armor'},
    ];
  };
  ...
  handlePlayerAttack(getAttackDescription) {
    ...
    if (this.currentEnemy.AC > this.player.getAttackValue()) {
  ...
  merchantShop() {
    ...
    // Prompt users to select an option
    inquirer.prompt({
      type: 'list',
  ...
  showWeaponInventory() {
    ...
    // Prompt to purchase weapon or go back
    inquirer.prompt({
      type: 'list',
      message: 'What would you like to purchase?',
      name: 'weaponChoice',
      choices: ['Weapon1', 'Weapon2', 'Go Back'],
    })
    .then(({ weaponChoice }) => {
      // Take user back to main shop menu
  ...
  showArmorInventory() {
    ...
    inquirer.prompt({
      ...
      message: 'What would you like to purchase?',
      name: 'armorChoice',
      choices: ['ArmorSet1', 'ArmorSet2', 'Go Back'],
    })
    .then (({ armorChoice }) => {
      if (armorChoice === 'Go Back') {
        this.merchantShop();
      } else {
        // console.log(armorChoice);
  ...
  purchaseEquipment(purchasedItem) {
    ...
    // check if purchased item is a weapon or an armor set
    if (purchasedItem.startsWith('Weapon')) {
      item = this.weaponData.find((weapon) => weapon.name === purchasedItem);
      console.log('Weapon item:', item);
      this.purchaseWeapon(item);
      // console.log('Player has bought a weapon!');
    } else if (purchasedItem.startsWith('Armor')) {
      item = this.armorData.find((armor) => armor.name === purchasedItem);
      console.log('Armor item:', item);
      this.purchaseArmor(item);
      // console.log('Player bought an armor set!');
    }

    this.player.gp -= item.price;

    // Return to main merchant menu
  };

  purchaseWeapon(item) {
    this.player.weapon = item.name;
    this.player.strength = this.player.strength + item.damage;
    this.player.ba = this.player.ba + item.Attack_Bonus;

    console.log(`You purchased ${item} for ${item.price}.`);
    console.log('Updated player stats:', this.player);
  }

  purchaseArmor(item) {
    this.player.armor = item.name;
    this.player.ac = this.player.ac + item.Armor_Bonus;
